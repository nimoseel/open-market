...
const Join = () => {
    ...
    const companyNumValidator = () => {
        ...
        if (
            company_num.value.length !== 0 &&
            !company_num.value.match(regex.companyNum)
        ) {
            company_num.setIsValid(false);
    ...
    return (
        <SC.ContentSection>
                ...
                <SC.InputWrapper>
                    <div>
                        <S.IncludeBtnDiv>
                            ...
                            <S.CheckBtn type="button" onClick={idCheck}>
                                중복확인
                            </S.CheckBtn>
                        </S.IncludeBtnDiv>
                    ...
                    <S.UserInfoDiv>
                        <S.InputDiv>
                            ...
                            <S.Input
                                type="text"
                                maxLength="5"
                                valid={user_name.isValid}
                                {...user_name}
                            />
                        </S.InputDiv>
                        <SC.ErrMsg valid={user_name.isValid}>
                            {user_name.error}
                        </SC.ErrMsg>

                        <S.JoinTitle htmlFor="mobile_1">
                            휴대폰 번호
                        </S.JoinTitle>
                        <S.GapDiv>
                            <Dropdown {...mobile_1} />
                    ...
                    {!isSelected && (
                        <S.StoreInfoDiv>
                            <S.IncludeBtnDiv>
                                <S.InputDiv>
                                    <S.JoinTitle htmlFor="company_num">
                                        사업자 등록번호
                                    </S.JoinTitle>
                                    <S.Input
                                        type="text"
                                        maxLength="10"
                                        valid={company_num.isValid}
                                        {...company_num}
                                    />
                                </S.InputDiv>
                                <S.CheckBtn
                                    type="button"
                                    width={122}
                                    onClick={companyNumCheck}
                                >
                                    인증
                                </S.CheckBtn>
                            </S.IncludeBtnDiv>
                            <SC.Err valid={company_num.isValid}>
                                {companyNumErr}
                            </SC.Err>
                            <S.InputDiv>
                                <S.JoinTitle htmlFor="company_name">
                                    스토어 이름
                                </S.JoinTitle>
                                <S.Input
                                    type="text"
                                    maxLength="20"
                                    valid={company_name.isValid}
                                    {...company_name}
                                />
                            </S.InputDiv>
                            <SC.ErrMsg valid={company_name.isValid}>
                                {companyNameErr}
                            </SC.ErrMsg>
                        </S.StoreInfoDiv>
                    )}
            ...
            {ValidWholeCheck() ? (
                <S.JoinBtn type={'active'} onClick={JoinCheck}>
                    가입하기
                </S.JoinBtn>
            ) : (
                <S.JoinBtn type={'disabled'} disabled>
                    가입하기
                </S.JoinBtn>
            )}
        </SC.ContentSection>

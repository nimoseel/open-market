...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        const fetchCartData = async () => {
            try {
                ...
                // 4. 체크 박스 값 확인
                const isAllItemsChecked = updatedCartList.every(
                    (item) => item.is_active,
                );
    ...
    const totalPrice = calculateTotal(
        cartLists.map((item) => item.is_active && item.price * item.quantity),
    );
    const totalFee = calculateTotal(
        cartLists.map((item) => item.is_active && item.shipping_fee),
    );

    const navigateToPayment = () => {
        const selected = cartLists.filter(
            (item) =>
                item.is_active === true && (item.order_kind = 'cart_order'),
        );
        navigate('/payment', {
            state: {
                order_data: selected,
    ...
    const updateAllCheckStatus = (updatedCartLists) => {
        const areAllItemsChecked = updatedCartLists.every(
            (item) => item.is_active,
        );
